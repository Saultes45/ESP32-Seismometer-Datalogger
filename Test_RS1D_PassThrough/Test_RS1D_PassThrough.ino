...
// Bump detection
const int32_t BUMP_THRESHOLD_POS = +1000;           
const int32_t BUMP_THRESHOLD_NEG = -1000;
...
void setup() {
  ...
  for (uint8_t cnt_fill; cnt_fill < 50; cnt_fill++)
  {
...
//Serial.println(testAcc);
  ...
  GeophoneSerial.setTimeout(10);// Set the timeout to 10 milliseconds (for findUntil)
...
void readRS1DBuffer(void)
{
  ...
  if (GeophoneSerial.available()) 
  {
    ...
    else
    {
      ...
      unsigned long howLongToWait = 30; // in [ms]
